...
public class HashTableDictionary<K,V> implements Dictionary<K,V> {
   ...
   * all key-value entries in the hash table.
	 * @return an iterable collection of all the entries in the dictionary
   */
  public Iterable<Entry<K,V>> entrySet() {
		int j=0;
		ArrayIndexList<Entry<K,V>> A = new ArrayIndexList<Entry<K,V>>();
   ...
   * all entries whose keys are equal to a given key k.
	 * @param the key of the entries looking for
	 * @return an iterable collection of all the entries with key k
   */
   ...
   * collection.
	 * @param the key of entries being removed
	 * @return an iterable collection of all entries removed
   */
  public Iterable<Entry<K,V>> removeAll(K k) throws InvalidKeyException {
		int i = findEntry(k);  // find this key first
    if (i < 0) 
			throw new InvalidKeyException("There is no such entry.");
		Iterable<Entry<K,V>> B = getAll(k);
		Iterator<Entry<K,V>> iterator = getAll(k).iterator();
