...
public class LinkedBinaryTree<E> implements BinaryTree<E> {
  ...
  /** 
	 ...
	 * @param pos the index list you want to insert the positions of nodes into
	 */
  ...
  /** 
	 * Creates a list of the nodes in the subtree of a node in postorder. 
	 * @param v the root of current subtree
	 * @param pos the index list you want to insert the positions if node into
	 */
  protected void postorderPositions(Position<E> v, IndexList<Position<E>> pos) {
    if (hasLeft(v))
      postorderPositions(left(v), pos);
    if (hasRight(v))
      postorderPositions(right(v), pos);
		pos.add(pos.size(), v);
  }

  /** 
	 * Returns an iterable collection of the elements in preorder. 
	 * @return an iterable collection of the elemtents in preorder
	 */
  public Iterable<E> preorderElements() {
		ArrayIndexList<Position<E>> positions = new ArrayIndexList<Position<E>>();
		ArrayIndexList<E> elements = new ArrayIndexList<E>();
		if (isEmpty()) {
			return elements;
		}else {
			preorderPositions(root(), positions);
			for (int i=0; i<positions.size(); i++)
				elements.add(i, positions.get(i).element());				
			return elements;
		}
  }
  ...
  /**
	 ...
	 * @return an iterable collection of the elements in postorder
	 */
